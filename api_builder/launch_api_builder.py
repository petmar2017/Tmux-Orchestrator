...
"""
    ...
    def launch_claude_in_window(self, window_num: int, agent_name: str, prompt_content: str):
        ...
        # Send the customized prompt
        # Split prompt into smaller chunks to avoid tmux buffer issues
        ...
        for i in range(0, len(lines), chunk_size):
            ...
            if chunk.strip():
                # Use send-claude-message.sh for reliability
                subprocess.run(["./send-claude-message.sh", 
                              f"{self.session_name}:{window_num}", 
                              chunk])
                time.sleep(1)
        
        print(f"  ✅ Launched {agent_name} in window {window_num}")
    ...
    def launch_all_agents(self):
        ...
        for window_num, prompt_file, agent_name in agents:
            # Read base prompt
